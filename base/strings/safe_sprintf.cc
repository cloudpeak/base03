...
// TODO(markus): Check if this restriction can be lifted.
  ...
  bool IToASCII(bool sign, bool upcase, int64 i, int base,
...
bool Buffer::IToASCII(bool sign, bool upcase, int64 i, int base,
                      char pad, size_t padding, const char* prefix) {
  ...
  if (sign && i < 0) {
    ...
    if (i == std::numeric_limits<int64>::min()) {
      ...
      minint = 1;
      num = static_cast<uint64>(-(i + 1));
    } else {
      // "Normal" negative numbers are easy.
      num = static_cast<uint64>(-i);
    }
  } else {
    num = static_cast<uint64>(i);
  }
