...
void Base16Encode(const base::StringPiece& input, std::string* output) {
  static char const alphabet[] = "0123456789abcdef";

  output->resize(input.size() * 2);

  for (unsigned i = 0, j = 0; i < input.size(); i++, j += 2) {
    uint8 byte = static_cast<uint8>(input[i]);
    (*output)[j] = alphabet[(byte >> 4) & 0xf];
    (*output)[j + 1] = alphabet[byte & 0xf];
  }
}

std::string Base16Encode(const base::StringPiece& input) {
  std::string output;
  Base16Encode(input, &output);
  return output;
}

std::string Base16Encode(const uint8* data, size_t len) {
  base::StringPiece input(reinterpret_cast<const char*>(data), len);
  std::string output;
  Base16Encode(input, &output);
  return output;
}
...
bool Contains(const base::StringPiece& str,
              const base::StringPiece& search, bool case_sensitive) {
  if(case_sensitive) {
    return str.contains(search);
  }
  base::StringPiece::const_iterator it =
    std::search(str.begin(),
                str.end(),
                search.begin(),
                search.end(),
...
bool StartsWithASCII(const base::StringPiece& str,
                     const base::StringPiece& search,
                     bool case_sensitive) {
  if (case_sensitive)
    return str.starts_with(search);
  else
    return base::strncasecmp(str.data(), search.data(), search.length()) == 0;
}
