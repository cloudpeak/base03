...
inline bool CaseInsensitiveEqual(base::StringPiece s, base::StringPiece t) {
  if (s.size() != t.size()) {
    return false;
  }
  return std::equal(
    s.begin(), s.end(), t.begin(), base::CaseInsensitiveCompareASCII<char>());
}
...
// Returns true if str contains search, or false otherwise.
BASE_EXPORT bool Contains(const base::StringPiece& str,
...
BASE_EXPORT bool StartsWithASCII(const base::StringPiece& str,
                                 const base::StringPiece& search,
                                 bool case_sensitive);
